...
class Bowl { //Monitor = critical section
    private int worms;
    ...
    //When methods in an monitor is marked with sync just one of those thread - method can run at the same time. 
    public synchronized void eatWorm(String birdID) throws InterruptedException {
        while (worms == 0) {
            wait(); //reminds me of semaphores
        }
        // Bird took a worm
        worms--;
